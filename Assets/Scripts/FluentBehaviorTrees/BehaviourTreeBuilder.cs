...
namespace FluentBehaviourTree
{
    ...
    public class BehaviourTreeBuilder
    {
		...
		/// <summary>
		/// Create an executeNtimes node that executes its child a maxium of N times.
		/// </summary>
		public BehaviourTreeBuilder ExecuteUntilSuccessNTimes(string name, int maxTimes)
		{
			var executeNNode = new ExecuteUntilSuccessNTimesNode(name,maxTimes);

			if (parentNodeStack.Count > 0)
			{
				parentNodeStack.Peek().AddChild(executeNNode);
			}

			parentNodeStack.Push(executeNNode);
			return this;
		}
